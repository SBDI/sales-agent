...
class Product(TypedDict): #Added this class
    ProductName: str
    Quantity: int
...
@tool
def create_order(
    products: List[Product], *, config: RunnableConfig # Modified this line
) -> Dict[str, str]:
    """
    Creates a new order (product purchase) for the customer.

     Arguments:
         products (List[Product]): The list of products to be purchased. # Modified this line
